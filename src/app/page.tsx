...
export default function HomePage() {
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-2xl flex-col gap-6 px-5 py-8">
      <header>
        <h1 className="text-2xl font-bold tracking-tight">유실물 촬영 도우미</h1>
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <label className="block">
            <span className="text-xs font-medium text-slate-600">접수번호</span>
            <input
              value={receiptNo}
              onChange={(e) => setMeta({ receiptNo: e.target.value })}
              placeholder="예: 2026-0912"
